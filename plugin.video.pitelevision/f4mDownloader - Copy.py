...
class f4mDownloader( object ):
	
	def __init__( self, url, proxy = None):
		self.outputfile = DIR_USERDATA + "/myfile.flv"
		self.url              = url
		self.proxy            = proxy
		...
		# Get the manifest
		self.manifest = self.getFile(self.url)
		# Parse the manifest
		self.parseManifest()
		segNum = self.segNum
		...
		while (fragNum < self.fragCount):
			...
			fragData = self.getFile(filename1)
			#self.audio = False
			#self.video = False
			if (flv is None):
	...
	def UpdateBootstrapInfo(self, bootstrapUrl):
		...
		while ((fragNum == self.fragCount) and (retries < 30)):
			bootstrapPos = 0
			try:
				bootstrapfile = self.getFile(bootstrapUrl)
			except:
				exit(-1)
	...
	def parseManifest( self ):
		...
		try :
			...
			# Duration
			self.duration     = 20#float( tree.find( "{http://ns.adobe.com/f4m/1.0}duration" ).text )
			self.media          = tree.findall( "{http://ns.adobe.com/f4m/2.0}bootstrapInfo" )[ -1 ]
			self.streamid = tree.findall( "{http://ns.adobe.com/f4m/2.0}media" )[ -1 ]
			# Bootstrap URL
		...
		try :
			bootstrapfile = self.getFile(self.urlbootstrap)
		except :
